...
const s = {
  ...
  tab: (active) => ({ padding: '9px 20px', borderRadius: '10px', border: `1px solid ${active ? 'var(--accent)' : 'var(--border)'}`, background: active ? 'var(--accent)' : 'transparent', color: active ? 'white' : 'var(--muted)', fontSize: '14px', cursor: 'pointer', fontFamily: 'var(--font)', fontWeight: active ? '500' : '400', position: 'relative' }),
...
export default function InventoryTracker() {
  ...
  function addSpool() {
    ...
    setSpools(prev => [...prev, {
      ...
      type: addType, brand: addBrand || 'Unknown', color: addColor || 'Unknown',
      totalWeight: w, remaining: w,
      price: gn(addPrice), date: addDate || todayStr(),
      history: [{ date: addDate || todayStr(), action: 'Purchased', grams: w, price: gn(addPrice) }]
    }])
    setAddBrand(''); setAddColor(''); setAddWeight(1000); setAddPrice(0)
  }

  function removeSpool(id) { setSpools(prev => prev.filter(s => s.id !== id)) }

  function applyUpdate() {
    if (!quId) return
    const amount = gn(quAmount)
    setSpools(prev => prev.map(s => {
      if (s.id != quId) return s
      const today = todayStr()
      if (quAction === 'use') {
        return { ...s, remaining: Math.max(0, s.remaining - amount), history: [...s.history, { date: today, action: 'Used', grams: -amount }] }
      } else if (quAction === 'buy') {
        const bp = gn(quPrice), bd = quDate || today
        return { ...s, remaining: s.remaining + amount, totalWeight: s.totalWeight + amount, price: s.price + bp, history: [...s.history, { date: bd, action: 'Purchased', grams: amount, price: bp }] }
      } else {
        return { ...s, remaining: Math.max(0, amount), history: [...s.history, { date: today, action: 'Manual set', grams: amount }] }
      }
    }))
    setQuAmount(0); setQuPrice(0)
  }

  function openModal(spool) { setModalSpool(spool); setModalQty(1) }
  ...
  function toggleSection(type) { setCollapsed(prev => ({ ...prev, [type]: !prev[type] })) }
  ...
  return (
    <div style={s.wrap}>
      ...
      <h1 style={s.title}>Inventory & Orders</h1>
      <p style={s.sub}>Track your filament stock by type. Get low stock alerts and manage your order list in one place.</p>

      <div style={s.metrics}>
        <div style={s.metric}><div style={s.metricLabel}>Total spools</div><div style={s.metricVal()}>{spools.length}</div></div>
        <div style={s.metric}><div style={s.metricLabel}>Total invested</div><div style={s.metricVal('var(--accent)')}>${totalInvested.toFixed(2)}</div></div>
        <div style={s.metric}><div style={s.metricLabel}>Avg $/gram</div><div style={s.metricVal()}>${avgPerG.toFixed(3)}</div></div>
        <div style={s.metric}><div style={s.metricLabel}>Low stock</div><div style={s.metricVal(lowCount > 0 ? '#854F0B' : '#3B6D11')}>{lowCount}</div></div>
      </div>

      <div style={s.tabs}>
        <button style={s.tab(tab === 'inventory')} onClick={() => setTab('inventory')}>🧵 Inventory</button>
      ...
      {tab === 'inventory' && (
        <>
          {/* ADD FORM */}
          <div style={s.card}>
            <div style={s.cardTitle}>Add filament</div>
            <div style={s.cardSub}>Add a new spool to your inventory</div>
            <div style={s.g5}>
              <div style={s.field}><label style={s.label}>Material</label>
                <select style={s.input} value={addType} onChange={e => setAddType(e.target.value)}>
                  {MAT_TYPES.map(t => <option key={t}>{t}</option>)}
                </select>
              </div>
              <div style={s.field}><label style={s.label}>Brand</label><input style={s.input} value={addBrand} onChange={e => setAddBrand(e.target.value)} placeholder="e.g. Bambu" /></div>
              <div style={s.field}><label style={s.label}>Color</label><input style={s.input} value={addColor} onChange={e => setAddColor(e.target.value)} placeholder="e.g. White" /></div>
              <div style={s.field}><label style={s.label}>Weight (g)</label><input style={s.input} type="number" value={addWeight} onChange={e => setAddWeight(e.target.value)} /></div>
              <div style={s.field}><label style={s.label}>Price ($)</label><input style={s.input} type="number" value={addPrice} step="0.01" onChange={e => setAddPrice(e.target.value)} /></div>
            </div>
            <div style={s.g2}>
              <div style={s.field}><label style={s.label}>Purchase date</label><input style={s.input} type="date" value={addDate} onChange={e => setAddDate(e.target.value)} /></div>
              <div style={{ display: 'flex', alignItems: 'flex-end' }}><button style={s.btn} onClick={addSpool}>+ Add to inventory</button></div>
            </div>
          </div>

          {/* QUICK UPDATE */}
          <div style={s.card}>
            <div style={s.cardTitle}>Quick update</div>
            <div style={s.cardSub}>Log usage or new purchase for an existing spool</div>
            <div style={s.g3}>
              <div style={s.field}><label style={s.label}>Select spool</label>
                <select style={s.input} value={quId} onChange={e => setQuId(e.target.value)}>
                  <option value="">— select —</option>
                  {spools.map(s => <option key={s.id} value={s.id}>{s.type} · {s.brand} · {s.color} ({s.remaining.toFixed(0)}g left)</option>)}
                </select>
              </div>
              <div style={s.field}><label style={s.label}>Action</label>
                <select style={s.input} value={quAction} onChange={e => setQuAction(e.target.value)}>
              ...
              <div style={s.field}>
                <label style={s.label}>{quAction === 'use' ? 'Amount used (g)' : quAction === 'buy' ? 'Amount purchased (g)' : 'Set remaining to (g)'}</label>
                <input style={s.input} type="number" value={quAmount} min="0" onChange={e => setQuAmount(e.target.value)} />
              </div>
            </div>
            {quAction === 'buy' && (
              <div style={s.g2}>
                <div style={s.field}><label style={s.label}>Price paid ($)</label><input style={s.input} type="number" value={quPrice} step="0.01" onChange={e => setQuPrice(e.target.value)} /></div>
                <div style={s.field}><label style={s.label}>Purchase date</label><input style={s.input} type="date" value={quDate} onChange={e => setQuDate(e.target.value)} /></div>
              </div>
            )}
            {quSpool && (
            ...
            <button style={s.btn} onClick={applyUpdate}>Apply update</button>
          </div>

          {/* INVENTORY BY TYPE */}
          {spools.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--muted)', fontSize: '14px', background: 'var(--white)', border: '1px solid var(--border)', borderRadius: '16px' }}>No filament added yet. Add your first spool above.</div>
          ) : (
            MAT_TYPES.map(type => {
              ...
              return (
                <div key={type} style={{ marginBottom: '16px' }}>
                  <div style={s.sectionHeader} onClick={() => toggleSection(type)}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '14px', fontWeight: '500' }}>
                      <span style={{ width: '10px', height: '10px', borderRadius: '50%', background: mc, border: '0.5px solid var(--border)', display: 'inline-block', flexShrink: 0 }} />
                      {type}
                      ...
                      <div key={sp.id} style={s.spoolCard}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
                          ...
                          <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                            ...
                            {isLow && <button style={s.btnOrder} onClick={() => openModal(sp)}>🛒 Order</button>}
                            <button style={s.btnEdit} onClick={() => { setQuId(sp.id); document.getElementById('qu-card')?.scrollIntoView({ behavior: 'smooth', block: 'center' }) }}>Edit</button>
                            <button style={s.btnDanger} onClick={() => removeSpool(sp.id)}>Remove</button>
                          </div>
                        </div>

                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '8px', marginBottom: '12px' }}>
                          {[['Remaining', sp.remaining.toFixed(0) + 'g'], ['Total bought', sp.totalWeight.toFixed(0) + 'g'], ['Total paid', '$' + sp.price.toFixed(2)], ['Per gram', '$' + perG]].map(([label, val]) => (
      ...
      {tab === 'orders' && (
        <div style={s.card}>
          ...
          <div style={s.cardSub}>Items added when stock runs low. Check off when ordered.</div>
          {orderList.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--muted)', fontSize: '14px' }}>No items in order list. Items appear here when you click Order on a low-stock spool.</div>
          ) : (
            <>
              {[...orderList.filter(o => !o.done), ...orderList.filter(o => o.done)].map(order => {
                const dotColor = getColorHex(order.color)
                return (
                  <div key={order.id} style={s.orderItem(order.done)}>
                    <input type="checkbox" checked={order.done} onChange={() => setOrderList(prev => prev.map(o => o.id === order.id ? { ...o, done: !o.done } : o))} style={{ width: '16px', height: '16px', cursor: 'pointer', flexShrink: 0 }} />
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: '12px', color: 'var(--muted)' }}>{order.qty}kg · Added {order.addedDate}</div>
                    </div>
                    <span style={{ fontSize: '12px', fontWeight: '500', color: order.done ? '#3B6D11' : '#854F0B' }}>{order.done ? 'Ordered ✓' : 'Pending'}</span>
                    <button style={s.btnDanger} onClick={() => setOrderList(prev => prev.filter(o => o.id !== order.id))}>Remove</button>
                  </div>
                )
              })}
            </>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={e => { if (e.target === e.currentTarget) setModalSpool(null) }}>
          <div style={{ background: 'var(--white)', border: '1px solid var(--border)', borderRadius: '16px', padding: '28px', width: '340px', maxWidth: '90vw' }}>
            ...
            <div style={{ fontSize: '13px', color: 'var(--muted)', marginBottom: '20px' }}>
              Stock is low ({modalSpool.remaining.toFixed(0)}g remaining). How much do you want to order?
            </div>
            <div style={s.field}>
              <label style={s.label}>Quantity</label>
              <select style={s.input} value={modalQty} onChange={e => setModalQty(parseInt(e.target.value))}>
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '16px' }}>
              <button style={{ ...s.btn, flex: 1 }} onClick={confirmOrder}>🛒 Add to order list</button>
              <button style={{ ...s.btnSm, flex: 0, padding: '10px 16px' }} onClick={() => setModalSpool(null)}>Cancel</button>
            </div>
          </div>
        </div>
      )}
